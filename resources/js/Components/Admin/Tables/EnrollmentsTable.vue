...
// Table Data
const items = computed(() => props.enrollments);
...
const deleteUserType = (item) => {
    itemId.value = item;
    isModalDangerActive.value = true;
};
const updateUserType = (iId, iN) => {
    itemId.value = iId;
    itemName.value = iN;
...
const deleteConfirmed = () => {
    useForm({
        id:itemId.value
    }).delete(route("admin.enrollment-types.destroy", itemId.value), {
        preserveScroll: true,
...
const enrollmentUpdateForm = useForm({
    id: itemId,
    newname: itemName,
});
...
const vForm$ = useVuelidate(formRule, enrollmentUpdateForm);
...
const updatesubmit = async () => {
    ...
    if (result) {
        enrollmentUpdateForm.put(route("admin.enrollment-types.update", itemId.value), {
            preserveScroll: true,
            onSuccess: () => {
                closeModal();
...
<template>
    ...
    >
        <CardBox
            ...
            is-hoverable
        >
            <input v-model="enrollmentUpdateForm.id" name="id" hidden />
            <FormField
            ...
            >
                <FormControl
                    v-model="enrollmentUpdateForm.newname"
    ...
    <table>
        ...
        <tbody>
            <tr v-for="enrollment in itemsPaginated" :key="enrollment.id">
                <TableCheckboxCell
                    v-if="checkable"
                    @checked="checked($event, enrollment)"
                />
                <td data-label="Name">
                    {{ enrollment.name }}
                </td>
                ...
                <td class="before:hidden lg:w-1 whitespace-nowrap">
                    <BaseButtons type="justify-start lg:justify-end" no-wrap>
                        <BaseButton
                            ...
                            :icon="mdiTrashCan"
                            small
                            @click="deleteUserType(enrollment.id)"
                        />
                        <BaseButton
                            color="success"
                            :icon="mdiPen"
